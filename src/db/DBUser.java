...
public class DBUser {
	...
	public static int DBrechercherUser(String log, String pass) {
		...
		try {
			result1 = state1
					.executeQuery("SELECT * FROM Etudiant where login='" + log + "' and password='" + pass + "'");
			result2 = state2
					.executeQuery("SELECT * FROM Professeur where login='" + log + "' and password='" + pass + "'");

			/*
			 * if(result1.next()) authentified = 1; else if(result2.next()) authentified =
			 * 2;
			 */
...
//voir dans openclassrooms comment se connecter en mode ecriture (constructeur avk parametres d'un objet statement)
	...
	public static void DBsaisirNote(Note note) {
		...
		try {
			state.executeUpdate("INSERT INTO Note (valeur, typeNote, matiere, loginEtudiant) VALUES ("
					+ note.getValeur() + ",'" + String.valueOf(note.getType()) + "','" + note.getMatiere().getNom()
					+ "','" + note.getLoginEtudiant() + "')");
			state.close();
		} catch (Exception e) {
			System.out.println("Echec de communication avec la base de donnees");
		}
